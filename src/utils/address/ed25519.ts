...
import GenericAddress, { encodeMnemonic, formatedSalt } from './base'
...
export default class DIOEd25519 implements GenericAddress {
  ...
  async generate() {
    ...
    const o = this.pkToDIOStruct(pku8, 3)
    const sk = dataview.u8ToHex(sku8)
    const pk = dataview.u8ToHex(pku8)
    const address = base32Encode(o.address, 'Crockford').toLowerCase() + ':' + this.algText
  ...
  private pkToDIOStruct(
    ...
    rollingCRC = 3,
    encryptMethod = 0x3, // ed25519 -> 3, sm2 -> 4
    salt = 1,
    alias?: string,
  ) {
